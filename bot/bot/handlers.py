...
async def run_digest(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        items, insights, week_label = generate_digest(topics, context=storage.get_context())
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        try:
            ...
            await update.message.reply_text(f"Дайджест собран, но не получилось сделать PDF: {exc}")
            return
